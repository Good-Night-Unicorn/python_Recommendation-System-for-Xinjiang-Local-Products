	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/techan?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('techan','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('techan','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
						...
						<div class="comment-item" v-for="item in infoList" :key="item.id" @mouseenter="discussEnter(item.id)"
							...
							<div class="comment-content-box">
								...
								<div class="comment-btn">
									<!-- <el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"#80BE1B","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}'>回复</el-button> -->
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('techan/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'techan', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'techan', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'techan', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/techan', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discusstechan/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discusstechan/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discusstechan/delete',[id]).then(res=>{
					...
					if (valid) {
						this.$http.get('orders/list', {params: {page: 1, limit: 1, status: '已完成', goodid: this.detail.id, userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
							if (res.data.code == 0 && res.data.data.list.length == 0) {
								...
								this.$http.get('discusstechan/page',{params: {
									page: 1,
									limit: 1,
									...
									this.$http.post('discusstechan/add', this.form).then(rs2 => {
										if (rs2.data.code == 0) {
			...
			addDiscussNum(type){
				...
				this.$http.get('discusstechan/list',{params: {
					page: 1,
					limit: 10000,
					refid: this.detail.id
				}}).then(rs=>{
			...
			getCartList() {
				this.$http.get('cart/list', {params: {userid: Number(localStorage.getItem('frontUserid')), tablename: 'techan', goodid: this.detail.id}}).then(res => {
					if (res.data.code == 0) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/techanAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此特产？') .then(_ => {
					this.$http.post('techan/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
...
<style rel="stylesheet/scss" lang="scss" scoped>
	.detail-preview {
		...
		.detail-tabs {
			border: 1px solid rgb(86, 133, 122);

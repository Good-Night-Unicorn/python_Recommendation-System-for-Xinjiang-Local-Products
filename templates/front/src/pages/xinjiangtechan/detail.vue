	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xinjiangtechan?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">销售量描述</div>
						<div class="text "  >{{detail.salestext}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">食品工艺</div>
						<div class="text "  >{{detail.shipingy}}</div>
					</div>
					<div class="item">
						<div class="lable">包装规格</div>
						<div class="text "  >{{detail.baozhuanggg}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xinjiangtechan','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('xinjiangtechan','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/xinjiangtechan', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/xinjiangtechanAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此新疆特产？') .then(_ => {
					this.$http.post('xinjiangtechan/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
